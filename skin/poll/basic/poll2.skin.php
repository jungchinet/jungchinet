...
<script>
function polling2(value){
	document.getElementById('gb_poll2').value=value;
	document.fpoll2.submit();
}
</script>

<!--새창 띄우지 않기 위한 hidden frame-->
<iframe src='about:blank' name='hdn2' frame='0' frameborder='0' style='width:0px;height:0px;display:none'></iframe>
...
<form name="fpoll2" method="post" action="<?=$g4[bbs_path]?>/poll_update2.php" onsubmit="return fpoll2_submit(this);" target="hdn2">
<input type="hidden" name="po_id" value="<?=$po[po_id]?>">
<input type="hidden" name="skin_dir" value="<?=$skin_dir?>">
<input type="hidden" name="gb_poll2" id="gb_poll2" value="">
...
<ol style='margin:2px;'><li><table width='100%' cellspacing='0' border='0'><tr>
	...
	for ($i=1; $i<=9 && $po["po_poll{$i}"]; $i++) {
		...
		echo "<td width='50%' style='height:70px;background-color:$scolor[$c];cursor:pointer;color:#ffffff' id='gb_poll2_$i' align='center' valign='middle' onclick='polling2($i);'>" . $po['po_poll'.$i] . "<br><span style='font-size:15pt;font-weight:bold;'>".$percent."</span><span style='font-size:8pt;'>%</span><br><span style='font-size:.5em;'>(".$po['po_cnt'.$i]."표)</span></td>";
		echo $p[$g];
		$num++;
...
function poll_result2(po_id)
{
    ...
    win_poll("../bbs/poll_result.php?po_id="+po_id+"&skin_dir="+document.fpoll2.skin_dir.value);
}
</script>
