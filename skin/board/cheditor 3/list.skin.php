...
<tr height=28 align=center>
    ...
    <? if ($is_checkbox) { ?><td width=5><INPUT onclick="if (this.checked) all_checked(true); else all_checked(false);" type=checkbox></td><?}?>
   <td>제목</td>
   <td width=50>글쓴이</td>
   <td width=80>카테고리</td>
   <!--<? if($allCnt){ ?>
   ...
   <? } ?>
    ...
    <td width=30><?=subject_sort_link('wr_good', $qstr2, 1)?>추천</a></td>
...
<tr height=28 align=center class='list_tr'> 
    ...
    <td align=left style='word-break:break-all;'>
        ...
        <? 
			echo "<nobr style='display:block; overflow:hidden; width:395px;'>";
			echo $list[$i][reply];
    ...
    <td align="center"><span style='font:normal 11px tahoma; color:#BABABA;'><?=$list[$i][wr_good]?></span></td>
...
<?
//게시판베스트 정렬방식 불러오기
$bsql="select bo_hot_list_basis from g4_board where bo_table='$bo_table'";
$brst=sql_fetch($bsql);
echo db_cache("popular_".$bo_table, 600, "latest_popular('thumb',$bo_table, 5, 70, '', 3, '{$brst['bo_hot_list_basis']}')");
?> 
