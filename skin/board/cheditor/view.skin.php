        ...
        <? if ($update_href) {
			...
			if($ip_data[0]==1){
				if($member[mb_id]==$view[mb_id] or $is_admin=='super' or ($is_admin=='board' and $premConfig[modiFlag])){
					echo "<a href=\"$update_href\"><img src='$board_skin_path/img/btn_modify.gif' border='0' align='absmiddle' alt='update'></a> ";
				}
			}else{
				if($member[mb_id]==$view[mb_id] or $is_admin=='super' or ($is_admin=='board' and $premConfig[modiFlag])){
					echo "<a href=\"$update_href\"><img src='$board_skin_path/img/btn_modify.gif' border='0' align='absmiddle' alt='update'></a> ";
...
<tr><td height=30>
    ...
    <? if(empty($ip_data[0])){ ?>
    <? if ($singo_href) { ?><span style="float:right;padding-right:5px;"><a href="<?=$g4[path]?>/bbs/singo_popin_update.php?bo_table=<?=$bo_table?>&wr_id=<?=$wr_id?>&wr_parent=<?=$view[wr_parent]?>"><img src='<?=$board_skin_path?>/img/icon_singo.gif' alt='singo'></a></span><? } ?>
