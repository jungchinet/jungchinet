...
<tr height=28 align=center> 
    ...
    <?
        if ($list[$i][file][0][file])
            $img = "$g4[data_path]/file/$bo_table/".$list[$i][file][0][file];// 첫번째 파일이 이미지라면
        else
            $img = "";

        if ($list[$i][is_notice] || !file_exists($img) ) {
		        $img = "";
		        $vtop = "";
		    } else {
    ...
    <td align=left style='word-break:break-all;<?=$vtop?>'>
        <?
        echo $nobr_begin;

        if ($img) {
          	echo "<a href='{$list[$i][href]}'><img src='" . thumbnail($img, $img_width, $img_height) . "' border='0' alt='$contents' align=absmiddle></a>";
        }
		    ?>
