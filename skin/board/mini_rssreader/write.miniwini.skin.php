...
<tr>
    ...
    <td><? if ($is_notice) { ?><input type=checkbox name=notice value="1" <?=$notice_checked?>>공지&nbsp;<? } ?>
        <? if ($is_html_) { ?><input onclick="html_auto_br(this);" type=checkbox value="<?=$html_value?>" name="html" <?=$html_checked?>><span class=w_title>html</span>&nbsp;<? } ?>
...
<tr>
    ...
    <td style='padding:5 0 5 0;'>
        ...
        -->
		<TEXTAREA id=wr_content class=w_textarea name=wr_content rows=10 style='width:95%' itemname="내용" required><?=$content?></TEXTAREA>
		<!-- miniwini웹에디터 삽입 항상 절대 경로로 할것 -->
		<?include ("$g4[bbs_path]/miniwini.visualEditor.php");?>
		<!--
		<textarea id=wr_content name=wr_content class=tx style='width:100%; word-break:break-all;' rows=14 itemname="내용" required 
