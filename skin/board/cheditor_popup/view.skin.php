...
<?
// wr_10을 출력
if ($view[wr_10]) $view[wr_10] = "예"; else $view[wr_10] = "아니오";
?>
<tr><td height=30>
    <table width=100% cellpadding=0 cellspacing=0>
    <tr><td width=100 height=30>&nbsp;&nbsp;팝업일시</td><td width=170>시작 <?=$view[wr_1]?></td><td width=250>종료 <?=$view[wr_2]?></td><td>&nbsp;</td></tr>
    <tr><td width=100 height=30>&nbsp;&nbsp;팝업위치(px)</td><td width=170>좌측 <?=$view[wr_3]?></td><td width=250>상단 <?=$view[wr_4]?></td><td>&nbsp;</td></tr>
    <tr><td width=100 height=30>&nbsp;&nbsp;팝업크기(px)</td><td width=170>가로 <?=$view[wr_5]?></td><td width=250>세로 <?=$view[wr_6]?></td><td>&nbsp;</td></tr>
    <tr><td width=100 height=30>&nbsp;&nbsp;쿠키일수</td><td width=170>일수 : <?=$view[wr_7]?></td><td width=250>출력 : <?=$view[wr_8]?></td><td>&nbsp;</td></tr>
    <tr><td width=100 height=30>&nbsp;&nbsp;팝업옵션</td><td width=170>맵 # : <?=$view[wr_9]?></td><td width=250>배경이미지반복 : <?=$view[wr_10]?></td><td>&nbsp;</td></tr>
</table></td></tr>
...
$back_img = "";
if ($view[file][0][view]) {
    $img_url = $view[file][0][path]."/".$view[file][0][file];
    if ($view[wr_9]) {  // 맵을 사용하는 경우
        // 기본값 정의
        $popup_padding = "0px";
        $popup_offset_x = 0;
    } else if ($view[link][1]) { // 맵을 안쓰면서 - 링크1에 값이 있는 경우 - 이미지로
        // 기본값 정의
        $popup_padding = "0px";
        $popup_offset_x = 0;        
        ...
        $back_img = "style=\"background-image:url('$img_url');{$back_img_repeat};width:{$view[wr_5]}\"";
    }
} else {
        $img_url = "";
        // 기본값 정의
...
?>
        ...
        <!-- 내용 출력 -->
        <table><tr><td valign="top" <?=$back_img?>>
        <? if ($view[wr_9]) {?>
            <span id="writeContents" class="ct lh" style="overflow:hidden;width:<?=$view[wr_5]?>;height:<?=$view[wr_6]?>"><img src="<?=$img_url;?>" usemap="#<?=$view[wr_9]?>"><?=conv_content_rev($view[wr_content]);?></span>
        <? } else if ($view[link][1]) { 
    		// wr_link1이 있는 경우에는 wr_link1으로 내용의 link를 걸어줍니다.
        $popup_link = $view[link][1];
  	    if ($view[link][2])
	          $target_link = " target='new' ";
	      else
	          $target_link = "";
        ?>
            <span id="writeContents" class="ct lh" style="overflow:hidden;width:<?=$view[wr_5]?>;height:<?=$view[wr_6]?>"><a href="<?=$popup_link?>" onfocus="this.blur()" style="text-decoration:none;" <?=$target_link?>><img src="<?=$img_url;?>"></a></span>
        <? } else {?>
            <span id="writeContents" class="ct lh" style="overflow:hidden;width:<?=$view[wr_5]?>;height:<?=$view[wr_6]?>"><?=$view[content];?></span>
        <? } ?>
        </td></tr></table>
