...
<tr><td height=30 style="padding:5px 0 5px 0;">
    <table width=100% cellpadding=0 cellspacing=0>
    <tr>
    	<td style='word-break:break-all; height:28px;'>&nbsp;&nbsp;<strong><span id="writeSubject">
      <?
        if ($view[wr_2]) {
            echo "<img src='$board_skin_path/img/icon_select.gif' align=absmiddle>";
        } else {
            if ($view[wr_datetime] > '2006-10-17')
                echo "<img src='$board_skin_path/img/icon_standby.gif' align=absmiddle>";
            else
                echo '&nbsp;';
        } 
      ?>
    	<? if ($is_category) { echo ($category_name ? "[$view[ca_name]] " : ""); } ?><?=cut_hangul_last(get_text($view[wr_subject]))?></span></strong>
    	</td>
    	<td width=100>
...
<tr><td height=30>
    ...
    &nbsp;&nbsp;<font style="font:normal 11px 돋움; color:#BABABA;">글쓴이 :</font> <?=$view[name]?><? if ($is_ip_view) { echo "&nbsp;($ip)"; } ?>&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <font style="font:normal 11px 돋움; color=#7A8FDB;">포인트</font> : <?=number_format($view[wr_1])?>점&nbsp;&nbsp;&nbsp;&nbsp;
...
<tr> 
    ...
    <div id="resContents" class="resContents">
        ...
        <!-- 내용 출력 -->
        <span id="writeContents" class="ct lh"><?=resize_content($view[content]);?></span>
