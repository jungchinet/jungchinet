...
if($doRss=='ok'){
	...
	while($r=mysql_fetch_array($rst)){
		...
		for($g=$cntNum;$g>=0;$g--){
			...
			$title = $item[$g]['title'].'<br>';
			$url   = $item[$g]['link'];
			$desc  = $item[$g]['description'];
			$compare=$title.$desc;
			...
			if($kwder=='ha'){
				...
				$is_kwd=0; //키워드 일치횟수 초기화 //키워드 일치횟수 초기화
				for($v=0;$v<$kwdCnt;$v++){
			...
			if(($kuku==1 or $kwder=='ho')){
				...
				//echo "<a href=$url>[$cate] ($trg) $title</a><br>$desc</li><br>";
					...
					$item[$g]['title'] = get_text(cut_str($item[$g]['title'], 255), 0);
					$stg = array( "&#038;", "&#039;", "quot;", "&lt;", "&gt;", "#39;" );
					$otg = array( "", "\'", "\"", "<", ">", "" );

					for($k=0; $k<count($stg); $k++){
						$item[$g]['title'] = str_replace($stg, $otg, $item[$g]['title']);
					}

					//$article[$i][cp_title] = addslashes(strip_tags($article[$i][cp_title]));
					$title=$item[$g]['title'] = addslashes($item[$g]['title']);
					$desc=$item[$g]['description'] = addslashes($item[$g]['description']);
