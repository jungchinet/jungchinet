...
<script>
function r2_change(num){
	document.getElementById('mb_6').value=num;
}
</script>
<?
		
		$dc="select count(*) from g4_member where mb_id='$mb[mb_id]' and mb_7 < now()";
		$dr=mysql_query($dc);
		...
		$r_list="<option value=''>선택안함</option>";
		$r_script="<script>function r_change(num){";
		while($r=mysql_fetch_array($rrst)){
			if($r[no]==$mb[mb_5]){ $selected="selected='selected'"; }else{ $selected=""; }
			...
			$r2rst=mysql_query($r2sql);
				...
				$r_script .= "	document.getElementById('r2').innerHTML=\"<div id='r2'><select id='r2' name='r2' onchange='r2_change(this.value);'><option value=''>선택안함</option>";
			while($r2=mysql_fetch_array($r2rst)){
				if($r2[no]==$mb[mb_6]){ $selected="selected='selected'"; }else{ $selected=""; }
				$r_script .= "<option value='$r2[no]' $selected>$r2[name]</option>";
			}			
				$r_script .= "</select></div>\"; }";
		}
		$r_script.=" if(!num){";
		$r_script.="document.getElementById('r2').innerHTML=\"<div id='r2' ><select name='r2' onchange='r2_change(this.value);'><option value=''>선택안함</option><select><div>\";";
		$r_script.="}";
		$r_script.="document.getElementById('mb_5').value=num;";
		$r_script.="document.getElementById('mb_6').value='';";
		$r_script.="}</script>";
...
<tr class='ht'>
    <td>지역선택</td>
    <td colspan='3'>
    <div style='float:left;'>1차지역 : <select id='r1' name='mb_5' onchange="r_change(this.value);"><?=$r_list?></select> / 2차지역 : </div><div id='r2'><select id='r2' name='mb_6' onchange='r2_change(this.value);'><option>선택안함</option></select></div>
            <script>r_change(<?=$mb[mb_5]?>);</script></td>
</tr>
<tr class='ht'>
    <td>주소</td>
    <td>
...
<tr class='ht'>
    ...
    <td>여분 필드 <?=$k?></td>
    <td><input type=text class=ed style='width:99%;' id='mb_<?=$k?>' name='mb_<?=$k?>' maxlength=255 value='<?=$mb["mb_$k"]?>'></td>
</tr>
<? } ?>
