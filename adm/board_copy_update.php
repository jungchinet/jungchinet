...
$row2 = sql_fetch($sql);
if ($row2)
    alert("{$target_table}은(는) 이미 게시판 TABLE이 있습니다.\\n\\nphpMyAdmin에서 해당 테이블을 정리후에 복사해주세요.");
...
// 게시판 정보 - 제목은 따로 업데이트, 위에서 하려면 좀 그렇네요. 필드의 순서 때문에
...
{
    $d = dir("$g4[data_path]/file/$bo_table");
    while ($entry = $d->read()) 
    {
        if ($entry == "." || $entry == "..") continue;

        /*
        @copy("$g4[data_path]/file/$bo_table/$entry", "$g4[data_path]/file/$target_table/$entry");
        @chmod("$g4[data_path]/file/$target_table/$entry", 0707);
        ...
        */
        ...
        if(is_dir("$g4[data_path]/file/$bo_table/$entry")){
            ...
            while ($entry2 = $dd->read()) {
                ...
                @copy("$g4[data_path]/file/$bo_table/$entry/$entry2", "$g4[path]/data/file/$target_table/$entry/$entry2");
                @chmod("$g4[data_path]/file/$target_table/$entry/$entry2", 0707);
                $copy_file++;
            }
            $dd->close();
        }
        else {
            @copy("$g4[data_path]/file/$bo_table/$entry", "$g4[path]/data/file/$target_table/$entry");
            @chmod("$g4[data_path]/file/$target_table/$entry", 0707);
            $copy_file++;
        }
    }
