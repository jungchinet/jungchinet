...
$g4[title] = $member[mb_nick] . "님의 $txt된 게시글";
...
$sql_search = " where wr_mb_id = '$member[mb_id]' ";
if ($w == "nogood") {
    $txt = "비추천";
...
// group by를 쓰면 count(*)의 의미가 바뀝니다.
// http://stackoverflow.com/questions/364825/getting-the-number-of-rows-with-a-group-by-query
$sql = " select count(*) as cnt $sql_common $sql_search ";
$row = sql_fetch($sql);
$row2 = sql_fetch(" select found_rows() as cnt ");
...
// 게시판 목록별로 정렬하기
$sql = " select distinct a.bo_table, b.bo_subject from $g4[board_good_table] a left join $g4[board_table] b on a.bo_table=b.bo_table where a.wr_mb_id = '$member[mb_id]' ";
$result = sql_query($sql);
$bo_list = array();
for ($i=0; $row=sql_fetch_array($result); $i++) {
        $bo_list[$i] = $row;
