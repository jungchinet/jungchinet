...
if ($po[po_end_date] != "0000-00-00") {
    ...
    if ($end_stamp < $g4['server_time']){
		...
		/*echo "<script>parent.poll_result3($_POST[po_id])</script>";*/
		$hello=0;
	}else{
		$hello=1;
	}
...
if (get_cookie("ck_po_id") != $po[po_id]) 
{
    ...
    if (!($search_ip || $search_mb_id)) 
    {
        ...
        if ($member[mb_id])
            $mb_ids .= $member[mb_id] . "\n";
        sql_query(" update $g4[poll_table] set po_cnt{$gb_poll3} = po_cnt{$gb_poll3} + 1, po_ips = '$po_ips', mb_ids = '$mb_ids' where po_id = '$po_id' ");
        $msg = "정상적으로 참여되었습니다.";
