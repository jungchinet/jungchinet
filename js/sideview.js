    ...
    function SideView(targetObj, curObj, mb_id, name, email, homepage, domain) 
    {
        ...
        //if (g4_is_member) {
            ...
            if (mb_id) 
                ...
                this.insertTail("memo", "<a href=\"javascript:win_memo('"+g4_path+"/" + g4_bbs + "/memo.php?kind=write&me_recv_mb_id="+mb_id+"', '" + mb_id + "', '" + domain + "');\">쪽지보내기</a>"); 
            // 메일보내기
            if (email) 
                this.insertTail("mail", "<a href=\"javascript:;\" onclick=\"win_formmail('"+mb_id+"','"+name+"','');\">메일보내기</a>");
        ...
        if (g4_bo_table) {
            if (mb_id) // 회원일 경우 아이디로 검색
                this.insertTail("mb_id", "<a href='"+g4_path+"/" + g4_bbs + "/board.php?bo_table="+g4_bo_table+"&sca="+g4_sca+"&sfl=mb_id,1&stx="+mb_id+"#board'>아이디로 검색</a>");
