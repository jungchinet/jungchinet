...
function goto_url($url)
{
    ...
    // 모든 경우에대해서 goto_url frame을 parent로 하니 이상한 일이...ㅠ.ㅠ..
    // 몇가지 프로그램에 대해서만, 그런 작업을 하게 했습니다.
    if ($g4[goto_url_pgm]) {
        $use_goto = 0;
...
// 검색 구문을 얻는다.
function get_sql_search($search_ca_name, $search_field, $search_text, $search_operator='and', $bo_table='', $area='', $cate3='')
{
    global $g4, $member;
	$area=trim($area);
	$cate3=trim($cate3);
    ...
    for ($i=0; $i<count($s); $i++) {
        ...
        if ($pp_id) {
            ...
            // sum 테이블이 없으면 insert를 합니다. 이때 검색어가 mb_id, mb_name인지 체크 해줍니다.
            if ( $pp_id && mysql_affected_rows() == 0 ) {
                // 아이디와 닉을 가려 냅니다. 이름은 검색을 할 수 없으니 제외하구요.
	...
	if ($area){
		...
		$str .= " $opop wr_1 like '%$area%' ";
	}
		
	if ($cate3){
		if($not_comment or $area){
			$opop='and';	
		}else {
			$opop='';	
		}
		$str .= " $opop wr_5 like '%$cate3%' ";	
	}
		
	if ($search_ca_name){
		if($not_comment or $area or $cate3){
			$opop='and';	
		}else {
			$opop='';	
		}
		$str .= " $opop wr_5='$search_ca_name' ";
	}
...
function view_file_link($file, $width, $height, $content="")
{
    ...
    if (preg_match("/\.($config[cf_image_extension])$/i", $file))
        {
        //불당팩 : 원본이미지를 보게 image_window2를 사용, width만주고 height는 설정하지 않음 -> skin에서 처리하도록 수정
...
// sql query의 오류 핸들링, http://dev.mysql.com/tech-resources/articles/guide-to-php-security-ch3.pdf
